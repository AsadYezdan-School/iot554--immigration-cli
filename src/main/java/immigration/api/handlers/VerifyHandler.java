...
public class VerifyHandler {
    ...
    public void shareCode(Context ctx) {
        ...
        if (req.orgId() == null) {
            ctx.status(400).json(new ErrorResponse(400, "Bad Request", "orgId is required"));
            return;
        }

        if (!req.lawfulPurposeConfirmed() || !req.dataProtectionConfirmed() || !req.nonDiscriminatoryConfirmed()) {
            verificationService.auditConfirmationRefusal(req.orgId());
            ctx.status(422).json(new ErrorResponse(422, "Unprocessable Entity",
                "All confirmations must be accepted to proceed"));
            return;
        }
        ...
        var outcome = verificationService.verifyByShareCode(
            req.orgId(), req.shareCode().toUpperCase(), req.dateOfBirth());
    ...
    public void document(Context ctx) {
        ...
        var outcome = verificationService.verifyByDocument(
            req.orgId(), req.documentNumber().toUpperCase(), docType);
        ctx.status(200).json(toResponse(outcome));
    }
